...
def random_downsimpling(data, num_cell):
    ...
    matrix = pd.DataFrame([np.array(range(data.shape[0])),data.obs['cell_type1']]).transpose()
    matrix.columns = ['index','cell_type1']
    sort_matrix = matrix.sort_values(['cell_type1','index'])
    t_sample = []
    groups = Counter(sort_matrix['cell_type1'])
    # i = 0
...
def umap_visual(data, title=None, save_path=None, label=None, asw_used=None):
    ...
    if data.shape[1] > 50:  # 判断数据是否是高维
        data = PCA(n_components=50).fit_transform(data)
    reducer = umap.UMAP(random_state=4132231, n_neighbors=30, min_dist=0.05, metric='cosine')
    embedding = reducer.fit_transform(data)


    if label is not None and isinstance(label[0], str):  ##
