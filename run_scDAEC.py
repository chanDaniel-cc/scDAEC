...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_heads', type=int, default=8, 
                        help='the number of attention heads, default:8')
    ...
    parser.add_argument('--training_epoch', type=int, default=300,                                
                        help='epoch of train stage, default:300')
    parser.add_argument('--clustering_epoch', type=int, default=100,
                        help='epoch of clustering stage, default:200')
    ...
    parser.add_argument('--knn', type=int, default=True,
    ...
    parser.add_argument('--name', type=str, default='Elliott',                                   ##name
                        help='name of input file(a h5ad file: Contains the raw count matrix "X")')
    parser.add_argument('--celltype', type=str, default='cell_type1',
    ...
    if args.celltype == "cell_type1":  
        celltype = adata.obs['cell_type1'].tolist()
    else:
        celltype = None
    ...
    if adata.shape[0] < args.max_num_cell:
        ...
        print(args)
        init_model = scDAEC(256, 64, 64, 256, n_input=args.n_input, n_z=args.n_z, heads=args.n_heads, device=device)
        ...
        if isinstance(adj, scipy.sparse.csr_matrix):
        ...
        if args.save_embedding is True:
            np.savetxt('./%s.csv'%(args.name), z.detach().cpu().numpy())
    ...
    #down-sampling input
        ...
        new_celltype = new_adata.obs['cell_type1']
        args.n_input = dataset.shape[1]
        print(args)
        init_model = scDAEC(256,64, 64, 256, n_input=args.n_input, n_z=args.n_z, heads=args.n_heads,device=device)
        ...
        copy_model = scDAEC(256,64, 64, 256, n_input=args.n_input, n_z=args.n_z, heads=args.n_heads, device=device)
